...
struct ColorPickerScreen: View {
    ...
    @State private var swiftUISRGBColor = ""
    ...
    @State private var isTextFieldFocusedSWI = false
    ...
    private var swiColorView: some View {
        HStack {
            NativeTextField(
                text: $swiftUISRGBColor,
                placeholder: "Color sRGB",
                font: .monospacedSystemFont(ofSize: textFieldFontSize, weight: .regular),
                isFocused: $isTextFieldFocusedSWI
            )
            .controlSize(.large)
            /*  .onChange(of: uiSRGBColor) {
             if
            ...
            Button("Copy SWI", systemImage: "doc.on.doc.fill") {
                swiftUISRGBColor.copyToPasteboard()
            }
            .labelStyle(.iconOnly)
            .symbolRenderingMode(.hierarchical)
            .buttonStyle(.borderless)
    ...
    var body: some View {
        VStack {
            ...
            if shownColorFormats.contains(.swiftuiSRGB) {
                swiColorView
            }
    ...
    private func updateColorsFromPanel(
        ...
        if !excludeSWI {
            swiftUISRGBColor = color.swiftUIColorString
        }
